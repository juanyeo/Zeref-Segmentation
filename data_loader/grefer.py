'''
Code From 'https://github.com/henghuiding/ReLA/blob/main/gres_model/data/datasets/grefer.py'
'''
...
class G_REFER:
    ...
    def __init__(self, data_root, dataset='grefcoco', splitBy='unc'):
        ...
        if dataset in ['grefcoco']:
            self.IMAGE_DIR = osp.join(data_root, 'train2014') # modified from 'images/train2014'
        else:
            raise KeyError('No refer dataset is called [%s]' % dataset)

        tic = time.time()

        # load refs from data/dataset/refs(dataset).json -> modified dataset/grefs/grefs(unc).json
        self.data = {}
        ...
        # load annotations from data/dataset/instances.json -> modified dataset/grefs/instances.json
            ...
            for ann_id in ann_ids:
                ...
                if type(ann['segmentation'][0]) == list:
                    ...
                    for seg in ann['segmentation']:
                        poly = np.array(seg).reshape((len(seg)//2, 2)) # modified from len(seg) / 2
                        polygons.append(Polygon(poly, True, alpha=0.5))
                        color.append(c)
                    #p = PatchCollection(polygons, facecolors=color, edgecolors=(1,1,0,0), linewidths=3, alpha=1)
                    #ax.add_collection(p)  # thick yellow polygon
                    p = PatchCollection(polygons, facecolors=color, edgecolors=(1,0,0,1), linewidths=2, alpha=0.5)
                    ...
                    color_mask = np.array([2.0,166.0,101.0,0.5])/255
                    for i in range(3):
                        img[:,:,i] = color_mask[i]
...
if __name__ == '__main__':
    ...
    for ref_id in ref_ids[40:60]:
        ref = refer.loadRefs(ref_id)[0]
        if len(ref['sentences']) < 2:
            continue
        refcat = ref['category_id'] if type(ref['category_id']) == list else [ref['category_id']]
        categories = ''
        for cat in refcat:
            if cat == -1:
                categories = 'No Target  '
                break
            categories += refer.Cats[cat] + ', '
        categories = categories[:-2]
        print('Category : %s' % categories)
        plt.figure()
        refer.showRef(ref, seg_box='seg')
        #plt.show()
        plt.savefig('cocog_'+str(ref_id)+'.png')